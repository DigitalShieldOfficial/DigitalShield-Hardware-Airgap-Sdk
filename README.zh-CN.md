...
联调中较常见的一类问题是：**热钱包已正确生成 UR 待签动画码，协议字段也无误，但 Digital Shield 硬件摄像头「扫不上」或扫很久仍无法完成解码**。这往往不是链上交易构造错误，而是**光学识读能力与单帧二维码密度不匹配**。
...
| 对照 | 同一台设备扫**稀疏**短码往往正常；换更大字号 / 更小分片后成功率明显上升 |

历史上硬件对接 OKX 等手机端展示的高密度动画 UR（部分 BTC 待签单帧可到约 **QR Version 12–17** 量级）时，就曾出现「先无检测 → 再 ECC 失败」的组合现象。
...
4. **与「协议错误」的区分**  
   若已能稳定扫完成，随后在 CBOR 解析报错，则属于**编码字段问题**（见上文 Buffer JSON 等），不是本节的光学问题。本节特指：**扫码阶段就过不去**。
...
- 固件裁剪分辨率、解码预处理等属设备能力边界；第三方接入**不能假设**硬件能稳定识别手机上「尽量少帧、尽量密」的交易所风格大码。

#### 小结

> **硬件扫热端交易码失败时，优先怀疑「单帧 QR 对摄像头来说太密」，而不是先改交易字段。**  
> 处理原则：**减小每帧载荷 → 增加帧数 → 保证码面清晰**，使每一帧落在设备摄像头可解码的密度范围内。
